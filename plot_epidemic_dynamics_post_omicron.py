...
new_variants_inf = [
    'None', 'Emerged from Omicron, more transmissible',
    'Emerged from WT, more severe',
    'New cluster, more severe'
]
...
var_ax_code = {
    'None': 0,
    'Emerged from Omicron, more transmissible':1,
    'Emerged from WT, more severe':2,
    'New cluster, more severe':3,
}

variant_timing = ['2022-02-25', '2022-04-25', '2022-08-25']

fig, axes = pl.subplots(4, 3, figsize=(10, 10), sharex='col', sharey='row')
...
for i, inf in enumerate(df['new_infections_by_variant']):
    if df['vaccine_boost'][i] == 0 and df['vaccine_prime'][i] == 0:
        if df['next_variant'][i] in new_variants_inf:
            ...
            l1, = ax.plot(x, inf.values[o_ind, o2_day_ind:] * factor_inf, c=colors[0], label='Omicron, wave 2')
...
pl.legend([omicron_lines[0], variant_lines[0], variant_lines[3], variant_lines[6]],
          ['Omicron, wave 2', 'Omicron-like, more transmissible', 'Delta-like', 'Escape variant, more severe'], loc='right', bbox_to_anchor=(2.7, 2.2),
          title='Variant')
axes[0,0].set_title(f'(A) 2 months after\nOmicron wave 1 peak')
axes[0,1].set_title(f'(B) 4 months after\nOmicron wave 1 peak')
axes[0,2].set_title(f'(C) 8 months after\nOmicron wave 1 peak')
axes[0,0].set_ylabel('% of Omicron peak')
axes[1,0].set_ylabel('% of Omicron peak')
axes[2,0].set_ylabel('% of Omicron peak')
axes[3,0].set_ylabel('% of Omicron peak')
